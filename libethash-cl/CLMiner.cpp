...
bool CLMiner::initEpoch_frk() {
	...
	//cllog << "OpenCL frk kernel";
        ...
        try {
            m_searchKernel = cl::Kernel(program, "search_frk");
        } catch (cl::Error const& err) {
...
void CLMiner::enumDevices_old(std::map<string, DeviceDescriptor>& _DevicesCollection)
{
    // Load available platforms
    vector<cl::Platform> platforms = getPlatforms();
    if (platforms.empty())
